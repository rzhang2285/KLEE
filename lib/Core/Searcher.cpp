...
/*
ExecutionState &HardwareSearcher::selectState() {
  ExecutionState *state = states.front();
  int pos = 0;
  for (std::vector<ExecutionState*>::iterator it = states.begin(); 
      it != states.end(); it++) {
    ExecutionState *s = *it;
    KInstruction *ki = s->pc;
  ...
  snapShot sn1 = createSnapshot(*state);
  if (snapshotHistory.find(sn1) != snapshotHistory.end()) {
     errs() << "hello1 " << states.size() << "\n";
     prune(pos);
     errs() << "hello2\n";
     return selectState(); 
  }
  else {
    snapshotHistory[sn1] = 1;
    return *state;
  }
}
*/

HardwareSearcher::HardwareSearcher(Executor &_executor) {
  bfsSwitchCounts = _executor.bfsSwitchCounts;
  dfsSwitchCounts = _executor.dfsSwitchCounts;
  lastStartTime = util::getWallTime();
  searchMode = 1;
  bfsCount = 0;
  dfsCount = 0;
...
HardwareSearcher::~HardwareSearcher() {
}

ExecutionState &HardwareSearcher::selectState() {
  if ((searchMode == 1) && 
      (bfsCount > bfsSwitchCounts)) {
    errs() << "Using BFS " << bfsCount << " times.\n";
    bfsCount = 0;
    searchMode = 1 - searchMode;
    lastStartTime = util::getWallTime();
  }
  if ((searchMode == 0) && 
      (dfsCount > dfsSwitchCounts)) {
    errs() << "Using DFS " << dfsCount << " times.\n";
    dfsCount = 0;
    searchMode = 1 - searchMode;
    lastStartTime = util::getWallTime();
  }
  ...
  if (searchMode == 1) {
    bfsCount ++;
//    errs() << "Haha: BFS\n";
    return *states.front(); // BFS
  }
  else {
    dfsCount ++;
//    errs() << "Haha: DFS\n";
    return *states.back(); // DFS
  }  
}


void HardwareSearcher::update(ExecutionState *current,
                         const std::vector<ExecutionState *> &addedStates,
                         const std::vector<ExecutionState *> &removedStates) {
  states.insert(states.end(),
                addedStates.begin(),
