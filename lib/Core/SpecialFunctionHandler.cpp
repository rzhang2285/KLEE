...
#define addDNR(name, handler) { name, \
  ...
  addDNR("klee_assert_success", handleAssertSuccess),
  addDNR("klee_assert_failure", handleAssertFailure),
...
void SpecialFunctionHandler::handleAssertSuccess(ExecutionState & state, 
    KInstruction *target, std::vector<ref<Expr> > &arguments) {
  assert(arguments.size()==1 && "invalid number of arguments to assert_success");
  if (executor.notAPreCond == true) {
    executor.terminateStateOnError(state, "Not A Precond ", Executor::AssertNotPrecond);
//    executor.notAPreCond = false;
  }
  else {
    executor.terminateStateOnError(state, "ASSERTION SUCCESS ", Executor::AssertSuccess);
  }
}

void SpecialFunctionHandler::handleAssertFailure(ExecutionState & state, 
    KInstruction *target, std::vector<ref<Expr> > &arguments) {
  assert(arguments.size()==1 && "invalid number of arguments to assert_failure");
  if (executor.notAPreCond == true) {
    executor.terminateStateOnError(state, "Not A Precond ", Executor::AssertNotPrecond);
    executor.notAPreCond = false;
  }
  else {
    executor.terminateStateOnError(state, "ASSERTION FAILURE ", Executor::AssertFailure);
  }
}
...
void SpecialFunctionHandler::handleAssertFail(ExecutionState &state,
  ...
  if (executor.fastValidation) {
    errs() << "FastValidationCountNotRst: " << executor.fastValidationCountNotRst << "\n";
    errs() << "LastValuesCountSame: " << executor.lastValuesCountSame << "\n";
//    executor.haltExecution = true;
    if ((executor.fastValidationCountNotRst <= executor.notRstLimit) && 
        (executor.lastValuesCountSame != (executor.lastValues.size()))){
      executor.validationCount ++;
      if (executor.validationCount > 0)
        executor.haltExecution = true;
    }
  }
  else {
    if (executor.haltWhenFired) {
      executor.haltExecution = true;
    }
  }
}
